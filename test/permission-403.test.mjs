 ...
 *
 * Measured on a real account (account-d@example.com, 27/08/2026): Anthropic
...
test('the 403 branch is classified as permanent, not retried on the same account', () => {
  ...
  assert.ok(/markAccountBlocked|blockKind: 'permission'|markAccountPermissionBlocked/.test(branch),
...
test('a permission block is not treated as a rate limit', () => {
  ...
  assert.ok(!/markAccountLimited\(/.test(branch),
    'a permission error must not be recorded as a rate limit: rate limits expire on ' +
    'their own and this one never will, so the dead account would return to rotation');
});
